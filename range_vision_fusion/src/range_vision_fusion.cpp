  ...
  const autoware_msgs::DetectedObjectArray::ConstPtr &in_range_detections)
{
  ...
  autoware_msgs::DetectedObjectArray range_in_cv_copy; // jaemin

  TransformRangeToVision(in_range_detections, range_in_cv, range_out_cv);
  range_in_cv_copy = range_in_cv; //jaemin
  // cv::Mat debug_image(1080, 1920, CV_8UC3, cv::Scalar(255, 255, 255));
  ...
  for (size_t i = 0; i < in_vision_detections->objects.size(); i++)
  {
    ...
    for (size_t j = 0; j < range_in_cv.objects.size(); j++)
    {
      ...
      // std::cout << "if loop : " << overlap.area() << " vs " << range_rect_area * overlap_threshold_ << " , " << vision_rect_area * overlap_threshold_ << std::endl;
      ...
      // jaemin debug
      // cv::rectangle(debug_image, range_rect, cv::Scalar(255, 0, 0), 3, 8, 0); // blue
      // cv::rectangle(debug_image, vision_rect, cv::Scalar(0, 0, 255), 3, 8, 0); // red    
    }//end for range_in_cv
  ...
  // cv::resize( debug_image, resized_image, cv::Size( debug_image.cols/3, debug_image.rows/3 ), 0, 0, CV_INTER_NN );
  // cv::imshow("resized_image", resized_image);
  // cv::waitKey(10);

  std::vector<bool> used_range_detections(range_in_cv_copy.objects.size(), false);
  ...
  // push back no used vision detection
  ...
  // add no used lidar_detection in camera image
  for(size_t i = 0; i < used_range_detections.size(); i++ )
  {
    if(!used_range_detections[i])
      fused_objects.objects.push_back(range_in_cv_copy.objects[i]);
  }

  //add also objects outside the image , lidar based
  ...
  const autoware_msgs::DetectedObjectArray::ConstPtr &in_range_detections)
{
  ...
  // std::cout << "detection : " << in_vision_detections->objects.size() << ", " << in_range_detections->objects.size() << std::endl;
  ...
  publisher_fused_objects_.publish(fusion_objects); // velodyne frame
  ...
  makeJskBoundingBoxes(fusion_objects);
  // std::cout << "range_vision_fusion callback runtime : " << ros::Time::now() - start << std::endl; 
}

void ROSRangeVisionFusionApp::makeJskBoundingBoxes(autoware_msgs::DetectedObjectArray &fusion_objects)
{
  jsk_recognition_msgs::BoundingBoxArray detected_objects;
  jsk_recognition_msgs::BoundingBox detected_object;

  detected_objects.header = fusion_objects.header;
  
  for(size_t i = 0; i < fusion_objects.objects.size(); ++i)
  {
    detected_object.header = fusion_objects.objects[i].header;
    detected_object.pose = fusion_objects.objects[i].pose;
    detected_object.dimensions = fusion_objects.objects[i].dimensions;

    detected_objects.boxes.push_back(detected_object);
  }

  publisher_fused_objects_jsk_.publish(detected_objects);
}
...
ROSRangeVisionFusionApp::InitializeROSIo(ros::NodeHandle &in_private_handle)
{
  ...
  in_private_handle.getParam("fused_topic_str", fused_topic_str); //ichthus : yjchoi
  ...
  // publisher_fused_objects_map_frame = node_handle_.advertise<autoware_msgs::DetectedObjectArray>("/tracked_objects_map_frame", 1);
  publisher_fused_objects_jsk_ = node_handle_.advertise<jsk_recognition_msgs::BoundingBoxArray>("/fused_objects_jsk", 1);
